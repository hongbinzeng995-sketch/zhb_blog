...
<template>
  <PageHeader v-if="getMessageTypeIsCard">
    <template #route>
      <el-popover placement="top-start" :width="110" trigger="hover"
    ...
    <div class="message-header">
      <div class="flex items-center !w-[100%] !h-[1.2rem] !z-[2000]">
        <TypeWriter class="type-writer" size="1.2rem" :typeList="'test'"></TypeWriter>
      </div>
      <div class="publish flex items-center">
        <el-popover placement="top-start" :width="110" trigger="hover" content="点我去发表留言">
          <template #reference>
            <svg-icon class="!z-[2000] cursor-pointer" name="publish" :width="4" :height="4" @click="userAddMessage"
              @mouseenter="showMessageInput = true"></svg-icon>
          </template>
        </el-popover>
      </div>
    </div>
  ...
  <div v-else class="relative !w-[100%] !h-[100vh] flex flex-col justify-center items-center">
    ...
    <div class="!mt-[3rem] flex items-center">
      <transition name="down" mode="out-in">
        <el-input v-model="message" class="!z-[2000] message-input !mr-[10px]" placeholder="一定要留下点什么～"
          v-if="showMessageInput" :maxlength="555" @blur="
            () => {
              showMessageInput = false;
            }
          " @keyup.enter="userAddMessage"></el-input>
      </transition>
